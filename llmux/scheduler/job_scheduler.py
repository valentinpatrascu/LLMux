...
from common.exceptions import GenerationTimeout, GenerationError, JobCancelledError
...
class Scheduler:
    ...
    async def store_inference_config(self, request_id: UUID, llm_engine: LLMEngine, worker_models: list[str]) -> None:
        await self.job_repository.update_job_record_details(request_id=request_id, llm_engine=llm_engine, worker_models=worker_models)

    async def mark_status(self, request_id: UUID, job_status: JobStatus) -> None:
        await self.job_repository.update_job_record_status(request_id=request_id, job_status=job_status)

    async def store_worker_model_outputs(self, request_id: UUID, worker_model_outputs: list[dict]) -> None:
        await self.job_repository.update_job_record_details(request_id=request_id, worker_model_outputs=worker_model_outputs)

    async def set_failure(self, request_id: UUID, job_status: JobStatus, failure: FailureDetails, finished_at: datetime) -> None:
        await self.job_repository.fail_job(request_id=request_id, job_status=job_status, failure=failure.model_dump(), finished_at=finished_at)

    async def set_finished_at(self, request_id: UUID, finished_at: datetime) -> None:
        await self.job_repository.update_job_record_details(request_id=request_id, finished_at=finished_at)
    ...
    async def dispatch(self, prompt: str, request_id: UUID, semaphore: Semaphore) -> None:
        ...
        async with semaphore:
            ...
            try:
                ...
                logger.exception(f"Job cancelled for request_id={request_id}.")
                try: 
                    await self.set_finished_at(request_id=request_id, finished_at=datetime.now(timezone.utc))
                except Exception:
